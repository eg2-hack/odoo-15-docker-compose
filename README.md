...
``` bash
curl -s https://raw.githubusercontent.com/eg2-hack/odoo-15-docker-compose/master/run.sh | sudo bash -s odoo-one 10015 20015
```

to set up first Odoo instance @ `localhost:10015` (default master password: `abdelmonem.me`)

and

``` bash
curl -s https://raw.githubusercontent.com/eg2-hack/odoo-15-docker-compose/master/run.sh | sudo bash -s odoo-two 11015 21015
```
...
* Default database password (**admin_passwd**) is `abdelmonem.me`, please change it @ [etc/odoo.conf#L60](/etc/odoo.conf#L60)
